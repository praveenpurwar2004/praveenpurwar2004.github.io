//code reference : https://gist.github.com/asielen/44ffca2877d0132572cb
d3.csv('data/multiline_data.csv', function(error, data) {
    data.forEach(function (d) {
        d.year = +d.year;
        d.acquired = +d.acquired;
        d.closed = +d.closed;
        d.operating = +d.operating;
        d.funding_total = +d.funding_total;
    });
    var chart = makeLineChart(data, 'year', {
        'Acquired': {column: 'acquired'},
        'Closed': {column: 'closed'},
        'Operating': {column: 'operating'},
        'Funding Total': {column: 'funding_total'}
    }, {xAxis: 'Years', yAxis: 'Fund'});
    chart.bind("#linechart");
    chart.render();
});
...
function makeLineChart(dataset, xName, yObjs, axisLables) {
    ...
    chartObj.margin = {top: 15, right: 60, bottom: 30, left: 110};
...
//Create scale functions
    ...
    //chartObj.xScale = d3.scaleLinear().range([0, chartObj.width]).domain([0, data.length]); //< Can be overridden in definition
...
// Render the chart
    chartObj.render = function () {
        ...
        chartObj.svg.append("g").attr("class", "y axis").call(chartObj.yAxis).append("text").attr("class", "label")
            .attr("transform", "rotate(-90)").attr("y", -110).attr("x", -340).attr("dy", ".71em").style("text-anchor", "middle").text(chartObj.yAxisLable);
        ...
        for (var y  in yObjs) {
            ...
            series.append('p').style("color",color(y)).text(y);
            yObjs[y].legend = series;
        }

//         Overlay to capture hover
        chartObj.svg.append("rect").style("fill", "rgba(255, 255, 255, 0.1)").attr("width", chartObj.width)
